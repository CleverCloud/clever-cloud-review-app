...
exports.toNameValueObject = toNameValueObject;
...
function parseRaw(rawInput = '', options = {}) {
    ...
    if (valueFirstChar === SIMPLE_QUOTE || valueFirstChar === DOUBLE_QUOTE) {
      ...
      if (isValueInvalid) {
        parsingErrors.push({
          ...
          name,
          pos: getPosition(rawInput, nextQuoteIdx + 1)
        });
        startIdx = nextNewLineIdx + 1;
      } else {
...
function parseRawJson(rawInput = '', options = {}) {
  let parsedInput;
  const parsingErrors = [];
  const {
    mode = ''
  ...
  const variablesWithNameValue = parsedInput.filter(({
    name,
    value
  }) => {
...
function toJson(variables) {
  ...
  const sortedVariables = variables.sort((a, b) => a.name.localeCompare(b.name)).map(({
    name,
    value
  }) => ({
    name,
...
function toNameEqualsValueString(variables, options = {}) {
  const {
    addExports = false
  } = options;
  return variables.sort((a, b) => a.name.localeCompare(b.name)).map(({
    name,
    value
  }) => {
