...
exports.AbstractLogsStream = void 0;

var _streamAbstract = require("./stream.abstract.js");
...
class AbstractLogsStream extends _streamAbstract.AbstractStream {
  ...
  prepareLogsSse() {
    return Promise.resolve({
      ...
      queryParams: (0, _pickNonNull.pickNonNull)({
        filter: this.filter,
        deployment_id: this.deploymentId
      }, ['filter', 'deployment_id'])
    }).then((0, _prefixUrl.prefixUrl)(this.apiHost)).then((0, _oauth.addOauthHeader)(this.tokens)).then(requestParams => {
      // prepare SSE URL's authorization query param
      const urlObject = new URL(requestParams.url);
      const qs = new URLSearchParams();
      Object.entries(requestParams.queryParams).forEach(([name, value]) => qs.set(name, value));
  ...
  async _openSource() {
    ...
    this._sse = this.createEventSource(url); // Sometimes, the EventSource is open but no messages are returned and no errors thrown.
    ...
    }, OPEN_TIMEOUT); // Wire SSE "message" to _onPing() call or "log" event

    this._sse.addEventListener('message', message => {
      clearTimeout(openTimeoutId);
      const parsedMessage = this.parseLogMessage(message);
      this.isPingMessage(parsedMessage) ? this._onPing(parsedMessage.heartbeat_delay_ms) : this.emit('log', parsedMessage);
    }); // Wire SSE "error" to _onError() call


    this._sse.addEventListener('error', error => {
      clearTimeout(openTimeoutId);
      this.isAuthErrorMessage(error) ? this._onError(new _streamAbstract.AuthenticationError('Authentication for the logs stream (SSE) failed')) : this._onError(error);
    });
  }
...
exports.AbstractLogsStream = AbstractLogsStream;