...
exports.ServerError = exports.HttpError = exports.NetworkError = exports.default = void 0;

var _customEventTarget = require("./custom-event-target.js");

var _oauth = require("../oauth.js");

var _sseFetchEventSource = require("./sse-fetch-event-source.js");
...
class CleverCloudSse extends _customEventTarget.CustomEventTarget {
  /**
  ...
  constructor(apiHost, tokens, retryConfiguration = {}, connectionTimeout) {
    ...
    this._retry = { ...DEFAULT_RETRY_CONFIGURATION,
      ...retryConfiguration
    };
  ...
  async _start() {
    try {
      ...
      if (this._tokens != null) {
        requestParams = await (0, _oauth.addOauthHeader)(this._tokens)(requestParams);
      }
      ...
      this._onError(new NetworkError('Server closed the response without a END_OF_STREAM event', {
        cause: error
      }));
    }
  }
  ...
  _canRetry(error) {
    ...
    const isErrorRetryable = error == null || error instanceof ServerError || error instanceof NetworkError || error instanceof HttpError && error.status >= 500;
  ...
  async _onOpen(response) {
    ...
    if (response.status !== 200) {
      if (contentType === _sseFetchEventSource.JSON_CONTENT_TYPE) {
    ...
    if (!contentType?.startsWith(_sseFetchEventSource.EVENT_STREAM_CONTENT_TYPE)) {
      ...
      case 'END_OF_STREAM':
        {
          try {
            ...
            this._onError(new ServerError(`Expect JSON for END_OF_STREAM event but got "${msg.data}"`, {
              cause: e
            }));
          }
      ...
      default:
        {
          try {
            ...
            this._onError(new ServerError(`Cannot transform ${msg.event} event`, {
              cause: e
            }));
          }
        }
...
exports.default = CleverCloudSse;
