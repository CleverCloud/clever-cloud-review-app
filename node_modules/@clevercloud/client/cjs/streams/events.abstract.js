...
exports.AbstractEventsStream = void 0;
...
class AbstractEventsStream extends _streamAbstract.AbstractStream {
  /**
  ...
  prepareEventsWs() {
    return Promise.resolve({
      ...
      const url = urlObj.toString(); // prepare message to auth WebSocket
  ...
  async _openSource() {
    const {
      url,
      authMessage
    } = await this.prepareEventsWs();
    this._ws = this.createWebSocket(url);

    this._ws.addEventListener('open', () => this._ws.send(authMessage));

    this._ws.addEventListener('message', message => {
      const parsedMessage = this.parseEventMessage(message); // Ignore socket_ready message
      ...
      if (this.isAuthErrorMessage(parsedMessage)) {
        return this._onError(new _streamAbstract.AuthenticationError('Authentication for the events stream (WS) failed'));
      }
    ...
    this._ws.addEventListener('error', error => this._onError(error)); // Save this listener so we can remove it easily in _closeSource
...
exports.AbstractEventsStream = AbstractEventsStream;