...
async function readBytes(stream, signal, onMessage) {
  // Setup parsers
  const onLine = getMessages(onMessage);
  const onChunk = getLines(onLine);

  if ('getReader' in stream) {
    const reader = stream.getReader(); // There's a bug in Node.js < 18.16.
...
function getLines(onLine) {
  let buffer; // current read position

  let position; // length of the `field` portion of the line

  let fieldLength;
  let discardTrailingNewline = false; // return a function that can process each incoming byte chunk:
  ...
  return function onChunk(arr) {
    ...
    const bufLength = buffer.length; // index where the current line starts
    ...
    while (position < bufLength) {
      ...
      onLine(buffer.subarray(lineStart, lineEnd), fieldLength); // we're now on the next line
...
function getMessages(onMessage) {
  ...
  const decoder = new TextDecoder(); // return a function that can process each incoming line buffer:
  ...
  return function onLine(line, fieldLength) {
    ...
    if (fieldLength > 0) {
      ...
      switch (field) {
        ...
        case 'retry':
          {
            const retry = parseInt(value, 10); // per spec, ignore non-integers
